...
const modificationRoutes = [
    { id: 1, path: "initial-input", component: <Modification />, name:"Main Case Details"},
...
const ModificationLayout = () => {
    ...
    const currentIndex = modificationRoutes.findIndex((step) =>
    ...
    useEffect(() => {
        setNextDisabled(currentIndex >= modificationRoutes.length - 1);
    }, [currentIndex, modificationRoutes.length]);

    // Update the title on URL change
    useEffect(() => {
        const activeStep = modificationRoutes.find((step) =>
    ...
    const handleNext = () => {
        if (!nextDisabled && currentIndex < modificationRoutes.length - 1) {
            navigate(`/filing/${basePath}/${modificationRoutes[currentIndex + 1].path}`);
        }
    };
