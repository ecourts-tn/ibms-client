...
const dischargeRoutes = [
    { id: 1, path: "initial-input", component: <DischargeSurety />, name:"basic_details"},
    { id: 2, path: "surety-detail", component: <SuretyDetails />, name:"Surety Detail" },
...
const DischargeLayout = () => {
    ...
    const currentIndex = dischargeRoutes.findIndex((step) =>
    ...
    useEffect(() => {
        setNextDisabled(currentIndex >= dischargeRoutes.length - 1);
    }, [currentIndex, dischargeRoutes.length]);

    // Update the title on URL change
    useEffect(() => {
        const activeStep = dischargeRoutes.find((step) =>
    ...
    const handleNext = () => {
        if (!nextDisabled && currentIndex < dischargeRoutes.length - 1) {
            navigate(`/filing/${basePath}/${dischargeRoutes[currentIndex + 1].path}`);
        }
    };
