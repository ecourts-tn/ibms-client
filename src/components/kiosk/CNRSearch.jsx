...
const CNRSearch = () => {
    ...
    const handleSubmit = async() => {
        try{
            ...
            setLoading(true)
            const response = await api.post("case/search/cnr-number/", {cino: form.cino})
                    ...
                    <div className="row">
                        <div className="col-md-8">
                            <input
                                name="cino"
                                label={t('cnr_number')}
                                className={`form-control ${errors.cino ? 'is-invalid' : ''}`}
                                value={form.cino}
                                onChange={(e) => setForm({...form, [e.target.name]: e.target.value})}
                                placeholder='CNR Number'
                            />
                            <div className="invalid-feedback">{ errors.cino }</div>
                        </div>
